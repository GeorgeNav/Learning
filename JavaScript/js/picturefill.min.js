      ...
      for (var i = 0, j = b.length; j > i; i++)
        if (
          ...
          "IMG" === c.nodeName.toUpperCase() &&
            (c[f.ns] || (c[f.ns] = {}), h.reevaluate || !c[f.ns].evaluated))
        ) {
          if ("PICTURE" === d.nodeName.toUpperCase()) {
            if ((f.removeVideoShim(d), (e = f.getMatch(c, d)), e === !1))
              continue;
          } else e = void 0;
          ("PICTURE" === d.nodeName.toUpperCase() ||
            (c.srcset && !f.srcsetSupported) ||
            (!f.sizesSupported && c.srcset && c.srcset.indexOf("w") > -1)) &&
            f.dodgeSrcset(c),
            e
              ? ((g = f.processSourceSet(e)), f.applyBestCandidate(g, c))
              : ((g = f.processSourceSet(c)),
                (void 0 === c.srcset || c[f.ns].srcset) &&
                  f.applyBestCandidate(g, c)),
            (c[f.ns].evaluated = !0);
        }
    }
    function e() {
      function c() {
        var b;
        a._picturefillWorking ||
          ((a._picturefillWorking = !0),
          a.clearTimeout(b),
    ...
    (f.ns = "picturefill"),
      ...
      (f.getWidthFromLength = function(a) {
        (a =
          ...
          (a = a.replace("vw", "%")),
          f.lengthEl ||
            ((f.lengthEl = b.createElement("div")),
            (f.lengthEl.style.cssText =
          ...
          f.lengthEl.offsetWidth <= 0 &&
            (f.lengthEl.style.width = b.documentElement.offsetWidth + "px");
      ...
      (f.types["image/gif"] = !0),
      (f.types["image/png"] = !0),
      (f.types["image/svg+xml"] = b.implementation.hasFeature(
      ...
      (f.findWidthFromSourceSize = function(a) {
        for (
          var b, c = f.trim(a).split(/\s*,\s*/), d = 0, e = c.length;
          e > d;
          d++
        ) {
          var g = c[d],
      ...
      (f.getCandidatesFromSourceSet = function(a, b) {
        for (
          var c = f.parseSrcset(a), d = [], e = 0, g = c.length;
          g > e;
          e++
        ) {
          var h = c[e];
      ...
      (f.getAllElements = function() {
        for (
          ...
          var g = c[d];
          ("PICTURE" === g.parentNode.nodeName.toUpperCase() ||
            null !== g.getAttribute("srcset") ||
            (g[f.ns] && null !== g[f.ns].srcset)) &&
            a.push(g);
        }
        return a;
      }),
