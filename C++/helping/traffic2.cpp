...
int main()
{
    ...
    rearangeCarInfo();
    isVisited = (bool *)mmap(NULL, sizeof(bool) * 50, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
...
cout<<"Child: "<<getpid()<<" "<<"parent: "<<getppid()<<endl;*/
            displayCarInfo(i);
            //Exiting the child process for new child to continue its execution.
            exit(0);
        }

        //other child process waits till the time the current car takes to go.
        sleep(carsinfo_array[i].time);
        // sleep(4);
    }
